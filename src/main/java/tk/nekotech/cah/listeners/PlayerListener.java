...
public class PlayerListener extends MasterListener {
    ...
    public void onMessage(final MessageEvent event) {
        ...
        if (message.length == 1 && message[0].equalsIgnoreCase("info")) {
            this.bot.sendMessage(event.getUser(), "You currently have " + player.getScore() + " awesome points. The black card is: " + this.cah.blackCard.getColored());
            this.bot.sendMessage(event.getUser(), "Your cards are: " + this.cah.getCards(player));
            return;
        }
        if (player.isCzar()) {
